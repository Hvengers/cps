...
#        if 'title != safe\n' in req.GET('http://hvenger.pythonanywhere.com/blog/'):
        '''
        serializer = SnippetSerializer(Snippet(title='aa'))
        print(serializer.data)
        print('hello')
        snippet = Snippet(title='safe\n')
        snippet.save()
        #snippet.delete()
        #r=requests.delete('http://127.0.0.1:8000/blog/none/')
        #print(r)
        serializer = SnippetSerializer(snippet)
        print(serializer.data)
        content = JSONRenderer().render(serializer.data)
        '''
        #r = requests.get('http://127.0.0.1:8000/blog/1/')
        #print(r)
        #r=requests.get(url='http://127.0.0.1:8000/blog/')
        #data =r.json()
        safeNum=0
        if 'isSafe' in req.POST:     #하나만 들어오진 않고, 무조건 다 들어오니 하나만 체크하면 됨.
            safeNum=req.POST['isSafe']
            safeNum=int(safeNum)
            if safeNum != 1:
                snippet = Snippet(title='safe\n')
                snippet.save()
        ...
        #r = requests.delete('http://hvenger.pythonanywhere.com/blog/')
        #print(r)
...
#        File(file=afile, files=test).save()
    ...
    if flag_safe ==1:
        ...
        file_loc="/home/Hvenger/cps/blog/static/image/"
